...
__all__ = ['birealnet18', 'birealnet34','birealnet18_uniadapt']
...
class birealnet18_uniadapt(BiRealNet):
    def __init__(self, block=BasicBlock, layers=[4,4,4,4], num_classes=1000, zero_init_residual=False):
        super().__init__(block, layers, num_classes, zero_init_residual)
        self.uniAdapt = False
        self.uniAdaptNet = None
        self.head = nn.Linear(512 + 512,102)
        self.adaptHeadOnly = False
    def freeze(self):
    ...
    def forward(self, x):
        if self.uniAdapt:
            ...
            adapt_out = self.uniAdaptNet(f_map)
            


            if self.adaptHeadOnly:
                return self.head(adapt_out)
            
            old_out = self.avgpool(f4)
            old_out = old_out.view(old_out.size(0), -1)
            vec_cat = torch.cat([old_out,adapt_out],dim=-1)
            return self.head(vec_cat)
