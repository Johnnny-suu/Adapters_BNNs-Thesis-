...
class BinarizeConv2d(nn.Conv2d):
    def __init__(self, *kargs, **kwargs):
        ...
        #Scaling factor for each output channel that we learn
        # self.alpha = nn.Conv2d(in_channels=self.out_channels,out_channels= self.out_channels,kernel_size=1, groups = self.out_channels,bias = False )
...
class BinarizeConv2d_binet(nn.Conv2d):
    def __init__(self, *kargs, **kwargs):
        super(BinarizeConv2d, self).__init__(*kargs, **kwargs)
        #Scaling factor for each output channel that we learn
        # self.alpha = nn.Conv2d(in_channels=self.out_channels,out_channels= self.out_channels,kernel_size=1, groups = self.out_channels,bias = False )
    ...
    def forward(self, input):
        ...
        self.weight.data=Binarize(self.weight.org)*scaling_factor
